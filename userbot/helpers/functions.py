...
async def admin_groups(AK):
    AKgroups = []
    async for dialog in AK.client.iter_dialogs():
...
async def yt_search(AK):
    try:
        AK = urllib.parse.quote(AK)
        html = urllib.request.urlopen(
            "https://www.youtube.com/results?search_query=" + AK
        )
...
async def extract_time(AK, time_val):
    if any(time_val.endswith(unit) for unit in ("m", "h", "d", "w")):
        unit = time_val[-1]
        time_num = time_val[:-1]  # type: str
        if not time_num.isdigit():
            AK.edit("Invalid time amount specified.")
            return ""
        if unit == "m":
...
async def waifutxt(text, chat_id, reply_to_id, bot, borg):
    ...
    AK = await sticcers[0].click("me", hide_via=True)
    if AK:
        await bot.send_file(int(chat_id), AK, reply_to=reply_to_id)
        await AK.delete()
